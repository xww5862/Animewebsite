...
public class VideoController {
    ...
    @RequestMapping("getMyVideoListByUserId")
    @ResponseBody
    public List<TVideo> getMyVideoListByUserId(Long userId) {
    ...
    //实现Dplayer播放的路径资源加载
    @RequestMapping("getVideoUrlByUserId")
    @ResponseBody
    public TVideo getVideoUrlByUserId(Long videoId) {
        return videoService.getVideoByVideoId(videoId);
    }


    @RequestMapping("search")
    public String searchVideo(@RequestParam(defaultValue = "1") int pageNum,
                              @RequestParam(defaultValue = "8") int pageSize,String videoTitle, Model model) {
        System.out.println(videoTitle+"videoTitle===================");
        PageHelper.startPage(pageNum, pageSize);
        List<TVideo> videoList = videoService.queryByVideoTitle(videoTitle);
        //获得 视频分页
        PageInfo<TVideo> pageInfo = new PageInfo<TVideo>(videoList);
//        System.out.println(pageInfo);
    ...
    public String uAddVideo(HttpSession session, @RequestParam String videoTitle,
        ...
        video.setUser(tUser);
        TState state = stateService.getStateByStateId(10L);
    ...
    public MsgResponse uploadVideo(@RequestParam MultipartFile file) {
        ...
        File newFile = new File(workplace + "/backend/src/main/resources/" + path);
...
//        TUser user = (TUser) session.getAttribute("user");
        ...
        File newFile = new File(workplace + "/backend/src/main/resources/static/vimages/" + fileName);
    ...
    public void getVideoImage(HttpServletResponse response, @PathVariable String iconPath) throws IOException {
        ...
        File file = new File(workpace + "/backend/src/main/resources/static/vimages/" + iconPath);
    ...
    @ResponseBody
    public MsgResponse deleteVideo(Long videoId){
        int res = videoService.deleteVideo(videoId);
        if (res!=0){
            return MsgResponse.success("删除视频成功",null);
        }else {
            return MsgResponse.success("删除视频失败",null);
        }
    }
    ...
    @ResponseBody
    public PageInfo<TVideo> getPageInfo(int pageNum, int pageSize,long videoTypeId) {

        //调用一个pageHelper的一个静态方法
        PageHelper.startPage(pageNum, pageSize);
        //视频记录
        List<TVideo> videoInfoDTOs = videoService.getVideoByVideoTypeId(videoTypeId);
    ...
    //分页视频2
    @RequestMapping("videoPageInfoTwo")
    @ResponseBody
    public PageInfo<TVideo> getPageInfoTwo(int pageNum, int pageSize,long userId) {

        //调用一个pageHelper的一个静态方法
        PageHelper.startPage(pageNum, pageSize);
        //视频记录
        List<TVideo> videoInfoDTOs = videoService.getVideoListByUserId(userId);
