...
public class MessageController {
    ...
    public MsgResponse readAll(Long userId) {

        return MsgResponse.success(messageService.readAll(userId, 7L), null);
    }


    @RequestMapping("deleteAllReadBtn")
    public MsgResponse deleteAllReadBtn(Long userId) {
        return MsgResponse.success(messageService.deleteAllReadBtn(userId,7L), null);
    }
    ...
    public MsgResponse addLetter(HttpSession session, @RequestParam String msgTitle, @RequestParam String msgContext, @RequestParam Long receiveUserId, @RequestParam Long msgTypeId) {
        ...
        addMsg = "发送用户为空";
        System.out.println("发送用户为空");
        return MsgResponse.fail(addMsg);
    }

    @GetMapping("/messageCount")
    public int messageCount(Long userId) {
        return messageService.msgCount(userId);
    }
